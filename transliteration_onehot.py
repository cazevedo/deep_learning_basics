...
def char2onehot(char_index, vocab, type=torch.float, grad=True):
    one_hot_char = [0]*len(vocab)
    one_hot_char[char_index] = 1
    return torch.tensor(one_hot_char, dtype=type, requires_grad=grad)

def word2onehot(word, vocab, type=torch.float, grad=True):
    onehot_word = word2index(word, vocab)
    for i, c in enumerate(onehot_word):
        onehot_word[i] = char2onehot(c, vocab, type=type, grad=grad)
    return onehot_word

def list2onehot(list_words, vocab, type=torch.float, grad=True):
    for i, word in enumerate(list_words):
        list_words[i] = word2onehot(word, vocab, type, grad)
    return list_words

def char2index(char, vocab):
    return np.where(vocab == char)[0][0]

def word2index(word, vocab):
    word_index = [0]*len(word)
    for i, c in enumerate(word):
        word_index[i] = char2index(c, vocab)
    return word_index

def list2index(list_words, vocab, type=torch.float, grad=True):
    for i, word in enumerate(list_words):
        list_words[i] = torch.tensor(word2index(word, vocab), dtype=type, requires_grad=grad)
    return list_words
...
def load_dataset(train_filepath='ar2en-train.txt', eval_filepath='ar2en-eval.txt', test_filepath='ar2en-test.txt'):
    ...
    if pickled_file.is_file():
        ...
        # convert to word index every sample entry
        X_train, X_dev, X_test = list2onehot(X_train, in_vocab, type=torch.float, grad=True), \
                                 list2onehot(X_dev, in_vocab, type=torch.float, grad=True), \
                                 list2onehot(X_test, in_vocab, type=torch.float, grad=True)
        ...
        print('Source Vocabulary : ( size = ', len(in_vocab), ')')
        print(in_vocab)
        print('Target Vocabulary : ( size = ', len(out_vocab), ')')
        print(out_vocab)
...
class UnitEncoderRNN(torch.nn.Module):
    def __init__(self, input_size, hidden_size, dropout_prob, device):
...
class UnitDecoderRNN(torch.nn.Module):
    def __init__(self, input_size, output_size, hidden_size, dropout_prob, device):
...
def decoder(decoding_obj, decoder_input, init_h, init_c, vocab_out, expected_output, criterion=None, teacher_forcing_prob=1.0):
    ...
    if np.random.rand() < teacher_forcing_prob:
        for index_char in expected_output:
            ...
            decoder_input = char2onehot(index_char, vocab_out, type=torch.float, grad=True)
    else:
        stop_char_index = char2index('EOW', vocab_out)
        ...
        for index_char in expected_output:
            ...
            # next iteration input
            decoder_input = char2onehot(output_char_index, vocab_out, type=torch.float, grad=True)
...
def train_rnn(encode_obj, decode_obj, X_train, y_train, X_dev, y_dev, vocab_in, vocab_out,
    ...
    # dec_h, dec_c = decode_obj.initHidden()
    ...
    for epoch in range(num_epochs):
        ...
        for index in rand_list:
            if reverse_input:
                ...
                y = np.fliplr(y[:-1].unsqueeze(0))[0]
                y = np.append(y, y_train[index][-1])
            ...
            # decode
            decoder_input = char2onehot(char2index('SOW', vocab_out), vocab_out, type=torch.float, grad=True)
    ...
    with torch.no_grad():
        for index in range(total):
            if reverse_input:
                ...
                X = X[:-1]
                X = X[::-1]
                X.append(X_dev[index][-1])
                y = np.fliplr(y[:-1].unsqueeze(0))[0]
                y = np.append(y, y_dev[index][-1])
                y = torch.tensor(y, dtype=torch.long, requires_grad=False)
            else:
                X, y = X_dev[index], y_dev[index]
...
def main(num_epochs = 40, learn_rate = 1e-4, reg_const = 1e-6, teacher_forcing_prob=0.5, reverse_input=True, generate_plot=True):
    ...
    encode_obj = UnitEncoderRNN(input_size=len(in_vocab), hidden_size=128, dropout_prob=0.0, device=processing_unit)
    decode_obj = UnitDecoderRNN(input_size=len(out_vocab), output_size=len(out_vocab), hidden_size=128, dropout_prob=0.0, device=processing_unit)
    ...
    pickled_file = Path("trained_model_onehot.pkl")
    with Path.open(pickled_file, 'wb') as f:
    ...
    file.write('ID: onehot_%i_%.2E_%.2E\t->\tTest ACC: %f \n' %
    ...
    if generate_plot:
        ...
        name = 'translit_onehot_acc_%i_%.2E_%.2E' % \
        ...
        name = 'translit_onehot_loss_%i_%.2E_%.2E' % \
               (num_epochs, learn_rate, reg_const)
