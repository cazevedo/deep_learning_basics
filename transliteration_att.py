...
def read_data(filepath):
    ...
    for line in f:
        ...
        if len(in_word) > max_len:
            max_len = len(in_word)
        if len(out_word) > max_len:
            max_len = len(out_word)
    ...
    return X, y, in_vocab, out_vocab, max_len
...
def load_dataset(train_filepath='ar2en-train.txt', eval_filepath='ar2en-eval.txt', test_filepath='ar2en-test.txt'):
    ...
    if pickled_file.is_file():
        ...
        X_train, y_train, in_vocab_train, out_vocab_train, max_len_train = read_data(train_filepath)
        X_dev, y_dev, in_vocab_dev, out_vocab_dev, max_len_dev = read_data(eval_filepath)
        X_test, y_test, in_vocab_test, out_vocab_test, max_len_test = read_data(test_filepath)
    ...
    return X_train, y_train, X_dev, y_dev, X_test, y_test, in_vocab, out_vocab, max_len
...
class BiEncoderRNN(torch.nn.Module):
    def __init__(self, input_size, vocab_size, hidden_size, dropout_prob, device):
        ...
        self.lstm = torch.nn.LSTM(input_size=input_size, hidden_size=hidden_size,
                                    num_layers=1, dropout=dropout_prob, bidirectional=True)

        self.ench2dech = torch.nn.Linear(self.hidden_size * 2, self.hidden_size)
        self.encc2decc = torch.nn.Linear(self.hidden_size * 2, self.hidden_size)
    ...
    def initHidden(self, rand=False):
        if rand:
            h = torch.randn(2, 1, self.hidden_size, device=self.device)
            c = torch.randn(2, 1, self.hidden_size, device=self.device)
        else:
...
class HiddenFF(torch.nn.Module):
    def __init__(self, hidden_size, device):
        super(HiddenFF, self).__init__()
        self.device = device
        self.hidden_size = hidden_size
    ...
    def forward(self, h_n, c_n):
        ...
        h_n = self.ench2dech(h_n)
        c_n = self.encc2decc(c_n)
        h_n = F.relu(h_n)
        c_n = F.relu(c_n)

        return h_n, c_n

class AttDecoderRNN(torch.nn.Module):
    def __init__(self, input_size, vocab_size, max_len, hidden_size, dropout_prob, device):
        super(AttDecoderRNN, self).__init__()
        self.device = device
        self.hidden_size = hidden_size
        self.max_len = max_len

        self.attn = torch.nn.Linear(self.hidden_size * 2 + input_size, self.max_len)
        self.attn_combine = torch.nn.Linear(self.hidden_size * 2 + input_size, input_size)
    ...
    def forward(self, input, h_n, c_n, encoder_outputs):
        input = input.view(1, 1, -1)
        embedded = self.embedding(input).view(1, 1, -1)
        # embedded = self.dropout(embedded)

        att_input = torch.cat((embedded, h_n, c_n), 2)
        att_input = self.attn(att_input)
        attn_weights = F.softmax(att_input, dim=2)
...
def encoder(encoding_obj, word, init_h, init_c, max_length):
    encoder_h = init_h
    encoder_c = init_c
    encoder_outputs = torch.zeros(max_length, 2*encoding_obj.hidden_size)
...
def decoder(decoding_obj, decoder_input, encoder_outputs, init_h, init_c, vocab_out, expected_output, criterion=None, teacher_forcing_prob=1.0):
    decoder_h = init_h
    decoder_c = init_c
    ...
    if np.random.rand() < teacher_forcing_prob:
        for index_char in expected_output:
            decoder_output, decoder_h, decoder_c, attn_weights = decoding_obj(Variable(decoder_input), decoder_h, decoder_c, encoder_outputs)
    ...
    return loss, outputs, decoder_h, decoder_c, attn_weights

def train_rnn(encode_obj, decode_obj, hidden_obj, X_train, y_train, X_dev, y_dev, vocab_in, vocab_out, max_len,
    ...
    if device != 'cpu':
        ...
        hidden_obj = hidden_obj.to(device)
    ...
    # hidden_optimizer = torch.optim.SGD(hidden_obj.parameters(), lr=eta, weight_decay=reg)
    ...
    hidden_optimizer = torch.optim.Adam(hidden_obj.parameters(), lr=eta, weight_decay=reg)
    ...
    for epoch in range(num_epochs):
        ...
        for index in rand_list:
            ...
            h, c = encode_obj.initHidden(rand=True)

            # encode
            encoder_outputs, h, c = encoder(encode_obj, X, h, c, max_len)

            # convert bidirectional hidden size to unidirectional hidden size
            h, c = hidden_obj(h,c)

            # decode
            decoder_input = char2index('SOW', vocab_out, type=torch.long, grad=False)
            loss, outputs, decoder_h, decoder_c, decoder_weights = decoder(decode_obj, decoder_input, encoder_outputs,
                                                                           h, c, vocab_out,
        ...
        word_accuracy = evaluate(encode_obj, decode_obj, hidden_obj, X_dev, y_dev, vocab_out, max_len, device, reverse_input=reverse_input)
        accuracies.append(word_accuracy)
        print('Epoch: %d, Total Loss: %f, Dev accuracy: %f' %
                    (epoch + 1, total_loss, word_accuracy) )

    return encode_obj, decode_obj, hidden_obj, accuracies, losses

def evaluate(encode_obj, decode_obj, hidden_obj, X_dev, y_dev, vocab_out, max_len, device, reverse_input):
...
def main(num_epochs = 40, learn_rate = 1e-4, reg_const = 1e-6, teacher_forcing_prob=0.5, reverse_input=False, generate_plot=True):

    X_train, y_train, X_dev, y_dev, X_test, y_test, in_vocab, out_vocab, max_len = load_dataset()
    ...
    emb_dim = 30
    hidden_size = 128
    encode_obj = BiEncoderRNN(input_size=emb_dim, vocab_size=len(in_vocab),
                              hidden_size=hidden_size, dropout_prob=0.0, device=processing_unit)
    decode_obj = AttDecoderRNN(input_size=emb_dim, vocab_size=len(out_vocab), max_len=max_len,
                                hidden_size=hidden_size, dropout_prob=0.0, device=processing_unit)
    hidden_obj = HiddenFF(hidden_size=hidden_size, device=processing_unit)

    trained_encoder, trained_decoder, trained_hidden, accuracies, losses = train_rnn(encode_obj, decode_obj, hidden_obj,
                                                                     X_train, y_train, X_dev, y_dev,
                                                                     in_vocab, out_vocab, max_len, device=processing_unit,
    ...
    pickled_file = Path("trained_model_att.pkl")
    with Path.open(pickled_file, 'wb') as f:
        pickle.dump( (trained_encoder, trained_decoder, trained_hidden, accuracies, losses), f)

    print('--Evaluating on the test set--')
    test_accuracy = evaluate(trained_encoder, trained_decoder, trained_hidden, X_test, y_test, out_vocab, max_len, processing_unit, reverse_input=reverse_input)
    print('Test accuracy: %f' % (test_accuracy))

    # save test accuracy to a file
    file = open('rnn_test_accuracy', 'a')
    file.write('ID: att_%i_%.2E_%.2E\t->\tTest ACC: %f \n' %
    ...
    if generate_plot:
        ...
        name = 'translit_att_acc_%i_%.2E_%.2E' % \
               (num_epochs, learn_rate, reg_const)

        plt.savefig('translit_att_%s.pdf' % name, bbox_inches="tight")
        ...
        name = 'translit_att_loss_%i_%.2E_%.2E' % \
               (num_epochs, learn_rate, reg_const)
